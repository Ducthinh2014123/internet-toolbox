...
export function readabilityScore(input: string): string {
  ...
  if (!text) throw new Error("Enter some text to analyze.");
  ...
  return [
    ...
    `Avg words/sentence: ${wordsPerSentence.toFixed(1)}`,
    `Avg syllables/word: ${syllablesPerWord.toFixed(2)}`,
    `Flesch Reading Ease: ${flesch.toFixed(1)} (${level})`,
    `Flesch-Kincaid Grade Level: ${Math.max(0, gradeLevel).toFixed(1)}`,
  ].join("\n");
}
